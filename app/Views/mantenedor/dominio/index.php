                        ...
                        <tr class="even pointer">
                            ...
                            <?php }
                                ...
                                if(in_array(session('idrol'),[1,2])){?>
                                    <button onclick="restablecer(<?=$row['dom_id']?>)" class="btn btn-warning btn-sm" title="Activar"><i class="bx bx-rotate-left"></i></button>
                            <?php }else{?>
...
<script>
    ...
    function eliminar(id) {
        alertify.confirm('¡Cuidado!','Estas por eliminar un registro, esta acción no se puede cancelar', function() {
            $.post( url + 'dominio/eliminar', {id: id, accion: 0}, function(response) {
                    if(response == 'ok'){
    ...
    function restablecer(id) {
        alertify.confirm('¡Cuidado!','Estas por activar un registro que fue desactivado', function() {
            $.post( url + 'dominio/eliminar', {id: id, accion: 1}, function(response) {
                    if(response == 'ok'){
