...
<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
    <h6 class="mb-0 text-uppercase">No Compuestos</h6>
    <div class="ms-auto">
        <div class="btn-group">
        <?php if (agregar()) { ?>
                <a href="compuesto/add" class="btn btn-primary btn-sm" style="color: #000;"><i class="fa fa-plus"></i> Nuevo</a>
            <?php } ?>
        </div>
    </div>
</div>
    ...
    <div class="card-body">
        <div class="col-md-12 col-sm-12">
            <div class="table-responsive">
                <table class="table table-striped table-bordered" id="tablaCompuesto">
                    <thead>
                        <tr class="headings">
                            <th class="column-title" style="text-align: center;">ITEM</th>
                            <th class="column-title" style="text-align: center;">NO COMPUESTO</th>
                    ...
                    <?php
                        ...
                        <tr class="even pointer">
                            ...
                            <td style="text-align: center;"><?=$row['com_nombre']?></td>
                            <td style="text-align: center;"><?=$row['com_descripcion']?></td>
                            <?php
                            ...
                            if($row['com_estado'] == 1){
                                if(editar()){
                                    $ff = 1;?>
                                    <a href="<?=base_url('compuesto/edit/'.$row['com_id'])?>" class="btn btn-success btn-sm"><i class="bx bx-edit"></i></a>
                            <?php }?>
                            <?php if(eliminar()){
                                $ff = 1;?>
                                <a href="#" onclick="eliminar(<?=$row['com_id']?>)" class="btn btn-danger btn-sm"><i class="bx bx-trash"></i></a>
                            <?php }
                            if($ff == 0){?>
                            ...
                            <?php }
                                ...
                                if(in_array(session('idrol'),[1,2])){
                                    $ff = 1;?>
                                    <button onclick="restablecer(<?=$row['com_id']?>)" class="btn btn-warning btn-sm" title="Activar"><i class="bx bx-rotate-left"></i></button>
                            <?php }else{?>
...
<script>
    ...
    function restablecer(id) {
        alertify.confirm('¡Cuidado!','Estas por activar un registro que fue desactivado', function() {
            $.post( url + 'compuesto/eliminar', {id: id, accion: 1}, function(response) {
                    if(response == 'ok'){
